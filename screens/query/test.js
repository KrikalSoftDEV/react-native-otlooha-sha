...
const RecitationDetailsScreen = ({route, navigation}) => {
  ...
  const playAudio = async url => {
    ...
    try {
      ...
      const path = `${fs.dirs.CacheDir}/detail_play_user_${Math.random()
        .toString(36)
        .slice(2)}.aac`;
  ...
  const playTeacherAudio = async url => {
    ...
    try {
      const {fs, config} = RNFetchBlob;
      const path = `${fs.dirs.CacheDir}/detail_play_teacher_${Math.random()
        .toString(36)
        .slice(2)}.mp3`;
      await config({fileCache: true, path}).fetch('GET', url);
      await audioRecorderPlayer.startPlayer(path);
  ...
  const teacherName = t?.teacherDetail?.firstName
    ? `${t.teacherDetail.firstName}${
        t.teacherDetail.lastName ? ' ' + t.teacherDetail.lastName : ''
      }`
    : 'Teacher';
  const teacherAvatarText = (
    t?.teacherDetail?.firstName ? t.teacherDetail.firstName[0] : 'T'
  ).toUpperCase();
  ...
  // Updated onReSendRecition with surahName, lineNo, recordedFile usage
...
console.log('worlimg')
    try {
      ...
      const textResult = await response.text();
        console.log(textResult,"TessxtResul")
      ...
      if (
        result.success ||
        (result.message &&
          result.message.toLowerCase().includes('uploaded successfully'))
      ) {
        setSurahName('');
        setLineNo('');
        setDescription('');
        ...
        <View style={styles.audioControls}>
          ...
          <View style={styles.controlButtons}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                const url = Array.isArray(item.voiceFile)
                  ? item.voiceFile[0]
                  : item.voiceFile;
                playAudio(url);
              }}>
          ...
          <>
            <View style={styles.respondedCard}>
              <View style={styles.respondedHeaderRow}>
                ...
                <TouchableOpacity
                  style={styles.reportTag}
                  onPress={() => setShowReportModal(true)}>
                  <Text style={styles.reportTagText}>Report</Text>
                </TouchableOpacity>
              </View>
              <Text style={styles.respondedDesc}>
                {Array.isArray(t.teachertextSuggestion)
                  ? t.teachertextSuggestion[0]
                  : ''}
              </Text>
              ...
              {t.teacherVoiceSuggestion &&
                t.teacherVoiceSuggestion.length > 0 && (
                  <>
                    <View style={styles.audioControls}>
                      <View style={styles.timeDisplay}>
                      ...
                      <View style={styles.controlButtons}>
                        ...
                        <TouchableOpacity
                          style={styles.playButton}
                          onPress={() =>
                            playTeacherAudio(t.teacherVoiceSuggestion[0])
                          }>
            ...
            {t.teacherVoiceSuggestion &&
              t.teacherVoiceSuggestion.length > 0 && (
                <TouchableOpacity
                  style={styles.downloadButton}
                  onPress={() =>
                    downloadAudio(t.teacherVoiceSuggestion[0], 'teacher')
                  }>
          ...
          <KeyboardAvoidingView
            ...
            <View style={styles.modalCard}>
              ...
              <Text style={styles.modalTitle}>
                Report an Issue with Teacher
              </Text>
              <Text style={styles.modalDesc}>
                Please let us know the reason for your report. Your feedback
                helps us keep the learning environment safe and respectful.
              </Text>
              <TextInput
                style={styles.input}
              ...
              <TouchableOpacity
                ...
                disabled={submitting}>
                <Text style={[styles.modalButtonText, {color: '#244E36'}]}>
                  Cancel
                </Text>
              </TouchableOpacity>
            </View>
          ...
          <View
            ...
            <View style={{width: '100%', marginBottom: 12}}>
              <View style={{marginTop: 10, marginBottom: 15}}>
                <Text style={{color: '#244E36', fontWeight: '600'}}>
                  Recitation Number
                </Text>
                ...
                <Text style={{fontSize: 16, color: '#000'}}>{item.chapterNo}</Text>
              </View>
                <View style={{marginTop: 10, marginBottom: 15}}>
                <Text style={{color: '#244E36', fontWeight: '600'}}>
                  Line  Number 
                </Text>
                <Text style={{fontSize: 16, color: '#000'}}>{item.pageNo}</Text>
              </View>
              ...
              {/* Checkbox to show/hide record & play */}
              <View
                style={{
                  flexDirection: 'row',
                  alignItems: 'center',
                ...
                <TouchableOpacity
                  onPress={() => setAllowRecord(!allowRecord)}
                  style={{
                    width: 18,
                    height: 18,
                    borderWidth: 1.1,
                ...
                <Text style={{color: '#244E36', fontSize: 15}}>
                  Do You Want to Record?
                </Text>
              </View>
            </View>

            {/* Conditionally show recording and playing controls */}
            {allowRecord && (
              <View
                style={{
                  alignItems: 'center',
                ...
                <TouchableOpacity
                  style={[
                    ...
                    isRecording ? styles.customRecordButtonActive : null,
                  ]}
                  onPress={toggleRecording}
                  activeOpacity={0.7}>
                  <Text
                    style={[
                      styles.customRecordIcon,
                      isRecording ? styles.customRecordIconActive : null,
                    ]}>
                    {isRecording ? '■' : '●'}
                  </Text>
                </TouchableOpacity>
                ...
                <View style={styles.customProgressBarBackground}>
                  <View style={styles.customProgressBarFill} />
                </View>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    !recordedFile ? styles.customPlayButtonDisabled : null,
                  ]}
            ...
            <TouchableOpacity
              ...
              <Text style={{color: '#269658', fontSize: 16, fontWeight: '700'}}>
                Close
              </Text>
            </TouchableOpacity>
          </View>
