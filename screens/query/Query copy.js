...
const onStartPlayVoice = async (voiceFileUrl, id) => {
  ...
  if (typeof voiceFileUrl === 'string') {
    audioUrl = voiceFileUrl;
  } else if (Array.isArray(voiceFileUrl) && voiceFileUrl.length > 0 && typeof voiceFileUrl[0] === 'string') {
    audioUrl = voiceFileUrl[0];
  } else if (voiceFileUrl && typeof voiceFileUrl.uri === 'string') {
    audioUrl = voiceFileUrl.uri;
  } else {
    console.error('Invalid audio URL:', voiceFileUrl);
    return;
  }
  ...
  try {
    if (playingVoiceId === id) {
      ...
      if (playingVoiceId && playingVoiceId !== id) {
        await audioRecorderPlayer.stopPlayer();
        audioRecorderPlayer.removePlayBackListener();
        setPlayingVoiceId(null);
      }
    ...
    return (
      <View style={styles.card}>
        ...
        <View style={styles.buttonsRow}>
          ...
          {/* Uncomment for recording functionality */}
          {/* {isRecording ? (
            <TouchableOpacity
